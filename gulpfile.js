...
function jsTask() {
	return src(
		...
		.pipe(concat('all.js'))
		.pipe(terser()) // plugin for minifying JavaScript files
...
function cacheBustTask() {
	...
	return src(['app/layout/overall_top.php', 'app/layout/overall_bottom.php'])
...
function bsWatchTask() {
	watch([files.appPath], browserSyncReload);
	watch(
		[files.scssPath, files.jsPath],
		{ interval: 1000, usePolling: true }, //Makes docker work
